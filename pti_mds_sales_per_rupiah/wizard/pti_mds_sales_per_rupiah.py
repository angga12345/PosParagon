...
class MdsSalesItemPerRupiah(models.TransientModel):
    _name = "mds.sales.item.rupiah.wizard"
    ...
    @api.multi
    def generate_excel_mds_sales_rupiah(self):        
        ...
        if filter_check == '1': 
            filename = 'Laporan Penjualan by Item / by  SKU per Rupiah per Store.xlsx'
        else:
            brand_id = self.brand_id.id
            brand = self.brand_id.name
            filename = 'Laporan Penjualan by Item / by  SKU per Rupiah per Store per-'+brand+'.xlsx'
        ...
        worksheet.merge_range('A1:C1', 'Laporan Penjualan by Item / by  SKU per Rupiah per Store', normal_left_bold)
        worksheet.merge_range('A3:B3','Start Period :', normal_left_unborder_bold)
        worksheet.merge_range('A4:B4', sp.strftime('%d %b %Y'), normal_left_unborder)
        worksheet.write('C3', 'End Period :', normal_left_unborder_bold)
        worksheet.write('C4', ep.strftime('%d %b %Y'),normal_left_unborder) 
        worksheet.write('G3', 'Print Date :', normal_left_unborder_bold)
        worksheet.write('G4', today.strftime('%d %b %Y %H:%M'), normal_left_unborder) 
        ...
        if filter_check == '1':
            self.env.cr.execute("""
              select 
                    pproduct.name_template as product, porder.shop_identifier_origin, porder.shop_identifier_period,
                    pline.price_unit as price, sum(pline.qty) as qty, pline.discount as discount, 
                    pline.price_unit*sum(pline.qty)*pline.discount/100 as total_disc, 
                    sum(pline.price_subtotal_rel) as total, sum(pline.price_subtotal_rel) * 0.1 as ppn
                from 
                ...
                where 
                    porder.date_order >= %s AND porder.date_order <= %s and pconfig.category_shop='shop_in_shop_mds'
                group by 
                    pproduct.name_template, 
                    porder.shop_identifier_origin, 
                    porder.shop_identifier_period,
                    pline.price_unit, pline.price_unit, pline.discount
                order by
                    pproduct.name_template
            ...
            self.env.cr.execute("""
              select 
                ...
                from 
                    ...
                    join product_product pproduct on pproduct.id = pline.product_id
                where 
                    porder.date_order >= %s AND porder.date_order <= %s and pconfig.tags=%s and pconfig.category_shop='shop_in_shop_mds'
                group by 
                    pproduct.name_template, 
                    porder.shop_identifier_origin, 
                    porder.shop_identifier_period,
        ...
        for pos in obj_pos:
            ...
            if pos[1] and pos[2]:
                sku = pos[1]
            elif pos[1] and not pos[2]:
                sku = pos[1]
            elif not pos[1] and pos[2]:
                sku = pos[2]
            elif not pos[1] and not pos[2]:
                sku = ""
            #manipulation sku in 8digit   
            if len(sku) > 8:
                sku = sku[4:][:-1]            
            worksheet.write(row+7 , 2, sku, normal_center)
            worksheet.write(row+7 , 3, pos[3], format_number)
            worksheet.write(row+7 , 4, pos[4], normal_center)            
            if pos[5]==0:
                worksheet.write(row+7 , 5, '', normal_center)
                worksheet.write(row+7 , 6, '', format_number)        
            else:
                worksheet.write(row+7 , 5, pos[5], normal_center)
                worksheet.write(row+7 , 6, pos[6], format_number)                                    
                sum_discount += pos[7]
            worksheet.write(row+7 , 7, pos[7], format_number)  
            worksheet.write(row+7 , 8, pos[8], format_number)

            sku_list.update({ inc : [sku,pos[7]]})            
            ...
            if len(obj_pos)-1==inc:
                ...
                worksheet.write(row+7 , 5, '', format_number_bold)        
                worksheet.write(row+7 , 6, sum_discount, format_number_bold)        
        ...
        worksheet.merge_range('A'+str(row+9)+':'+'B'+str(row+9), 'Taxes (Rp)', normal_left_bold)     
        worksheet.merge_range('A'+str(row+10)+':'+'B'+str(row+10), 'PPN 10%', normal_left_unborder)
        worksheet.write(row+9, 7, sum_all_ppn, format_number_unborder)                            
        worksheet.merge_range('A'+str(row+11)+':'+'B'+str(row+11), 'Sales by SKU', normal_left_bold)
        for key, value in sku_values.iteritems():
            row += 1
            worksheet.merge_range('A'+str(row+11)+':'+'B'+str(row+11), key, normal_left_unborder)
            worksheet.write(row+10, 7, value, format_number_unborder)            
                    
        worksheet.merge_range('A'+str(row+12)+':'+'B'+str(row+12), 'Total Sales (Rp)', normal_left_bold)                        
        worksheet.write(row+11, 7, sum_total, format_number_bold_minbord)
        worksheet.merge_range('A'+str(row+13)+':'+'B'+str(row+13), 'Total Discount (Rp)', normal_left_bold)                        
        worksheet.write(row+12, 7, sum_discount, format_number_bold_minbord)                                            
        workbook.close()
        ...
        bz.close()
        form_res = ir_model_data.get_object_reference('pti_mds_sales_per_rupiah', 'mds_sales_item_per_rupiah_wizard_view')
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'mds.sales.item.rupiah.wizard',
            'res_id': self.id,
            'view_id': False,
            'views': [(form_id, 'form')],
