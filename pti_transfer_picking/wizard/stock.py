...
class stock_backorder_confirmation(models.TransientModel):
    ...
    @api.multi
    def fast_process(self, cancel_backorder=False):
        self.ensure_one()
        for pack in self.pick_id.pack_operation_ids:
            if pack.qty_done > 0:
                pack.product_qty = pack.qty_done
            else:
                pack.write({'picking_id': False})
        self.pick_id.do_fast_transfer()
        if cancel_backorder:
    ...
    @api.multi
    def quick_process_cancel_backorder(self):
        self.ensure_one()
        self.fast_process(cancel_backorder=True)
