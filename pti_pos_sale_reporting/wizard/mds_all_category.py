...
class report_mds_all_category(osv.osv_memory):
    _name = "report.mds.all.category"
...
#         data['form'] = self.read(cr, uid, ids, parcel_ids, context=context)[0]
    ...
    def _excel_report(self, cr, uid, ids, data, context=None):
        ...
        if filter_check=='1':
            filename = 'Total Penjualan Per Kategori.xlsx'
        else:
            brand_id=check.brand_id.id
            brand=check.brand_id.name
            filename = 'Total Penjualan Per Kategori Per-'+brand+'.xlsx'
#         filename = 'Total Penjualan MDS Nasional.xlsx'
        ...
        if filter_check=='1':
            cr.execute("""
            select distinct
                pt.id , pt.name, temp.price_unit, sum(temp.qty) as Qty, sum(temp.price) as Nominal, temp.discount
            from
                (select 
                    prod.id, prod.product_tmpl_id, pol.qty, pol.price_unit, pol.price_subtotal_rel as price, pol.discount as discount
                from 
                ...
                where
                    po.date_order >= %s and po.date_order <= %s and pc.category_shop = 'shop_in_shop_mds') as temp
            join
                product_template as pt on pt.id=temp.product_tmpl_id
            group by
                pt.id,pt.name, temp.id, temp.price_unit, temp.discount
            order by
                pt.name
            ...
            from
                ...
                join 
                    product_product as prod on prod.id=pol.product_id
                where
                    po.date_order >= %s and po.date_order <= %s and pc.tags=%s and pc.category_shop = 'shop_in_shop_mds') as temp
            join
                product_template as pt on pt.id=temp.product_tmpl_id
            group by
                pt.id , pt.name, temp.id, temp.price_unit, temp.discount
        ...
        for prod in prod_param:
            ...
            join pos_product_category_product_template_rel as pcpt on pt.id=pcpt.product_template_id
            join pos_product_category as pc on pc.id=pcpt.pos_product_category_id
            where pt.id=%s
            order by pt.name
            """,(product_id,))
            categ=cr.fetchall()
...
#                 j+=1
        ...
        form_res = ir_model_data.get_object_reference(cr, uid, 'pti_pos_sale_reporting', 'report_mds_all_category_view')
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'report.mds.all.category',
            'res_id': ids[0],
            'view_id': False,
            'views': [(form_id, 'form')],
