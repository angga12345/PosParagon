...
class LapRetur(models.TransientModel):
    _name = "lap.retur"
    ...
    def get_data(self, start_period, end_period, location_id, location_dest_id):
        self.env.cr.execute("""
        select sp.name, sp.date, pp.default_code, pp.barcode, pp.name_template, round(((pt.list_price * (act.amount / 100)) + pt.list_price), 0), spo.product_uom_qty,
            ...
            """, (location_id, location_dest_id, start_period, end_period))
    ...
    def generate_lap_retur(self):
        for this in self:
            ...
            records = this.get_data(start_period + " 00:00:00", end_period + " 23:59:59", location_id.id, location_dest_id.id)
            filename = 'lap_retur.xlsx'
            ...
            worksheet = workbook.add_worksheet("Retur")
...
#             dates1=str(today).rpartition(':')[0]
#             times = datetime.strptime(today, "%Y-%m-%d %H:%M:%S")
            ...
            worksheet.write('B5', 'No Nota Retur',normal_bold_border)
            worksheet.write('C5', 'Tgl Retur dari Counter', normal_bold_border)
            worksheet.write('D5', 'Tgl Retur Diproses', normal_bold_border)
            worksheet.write('E5', 'Nama Driver', normal_bold_border)
            worksheet.write('F5', 'Nama BA', normal_bold_border)
            worksheet.write('G5', 'Tgl Input Barang Retur', normal_bold_border)
            worksheet.write('H5', 'Jam Input Barang Retur', normal_bold_border)
            ...
            bz.close()
            form_res = ir_model_data.get_object_reference("pti_finance_report", "lap_retur_wizard_view")
