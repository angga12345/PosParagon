...
class LaporanPromoDiscPerStoreReportWizard(models.TransientModel):
    _name = "laporan.promo.disc.per.store.report.wizard"
    ...
    def generate_excel(self):
        for this in self:
                ...
                if max_reward_id != max_price[9]:
                    for max_price_product in product_max_prices:
                        if max_price_product[3] == max_price[9]:
                            ...
                            sum_total += (max_price_product[2] * max_price_product[1])
                            sum_ppn += ((max_price_product[2] * max_price_product[1]) * 0.1)
                            row += 1
                worksheet.write(row+8, 0, max_price[0] + ' - ' + max_price[1], normal_left_border)
            ...
            for special_prices in special_pricess:
                for product in product_special_prices:
                    if product[3] == special_prices[5]:
                        ...
                        sum_total += (product[2] * product[1])
                        sum_ppn += ((product[2] * product[1]) * 0.1)
                        row += 1
                worksheet.write(row+8, 0, special_prices[0] + ' - ' + special_prices[1], normal_left_border)
            ...
            bz.close()
            form_res = ir_model_data.get_object_reference('lap_penj_promo_disc_per_store', 'laporan_promo_disc_per_store_report_wizard_view')
            form_id = form_res and form_res[1] or False
            return {
                'name': ('Download XLS'),
                'view_type': 'form',
                'view_mode': 'form',
                'res_model': 'laporan.promo.disc.per.store.report.wizard',
                'res_id': this.id,
                'view_id': False,
                'views': [(form_id, 'form')],
