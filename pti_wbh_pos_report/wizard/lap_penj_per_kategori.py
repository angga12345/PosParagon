...
class report_wbh_store_category(osv.osv_memory):
    _name = "report.wbh.store.category"
...
#         data['form'] = self.read(cr, uid, ids, parcel_ids, context=context)[0]
    ...
    def take_value_product_from_query(self, cr, uid, date_from, date_to, shop, context=None ):
        cr.execute("""
        select distinct
        ...
        from
            ...
            where
            po.date_order >= %s and po.date_order <= %s and pc.category_shop='stand_alone' and pol.price_unit>0 and pc.stand_alone_categ='wbh') as temp
        join
    ...
    def take_value_product_category(self,cr,uid,product_name):
        cr.execute("""
    ...
    def _excel_report(self, cr, uid, ids, data, context=None):
        ...
        worksheet.merge_range('E3:F3','Print Date :', bold_left)
        dt=datetime.now()
        worksheet.merge_range('E4:F4', dt.strftime('%d %b %Y %H:%M'),normal_left)
        ...
        rows=row+7
        obj = self.take_value_product_from_query(cr, uid, date_from, date_to, shop, context=context)
        print obj
...
#             nominal_after_discount = prod[4] - disc_value
            worksheet.write(rows-1,11, prod[4], format_number)
            
            product_name=prod[6]
            
            categ = self.take_value_product_category(cr,uid,product_name)
            print categ
            j=0
...
#                 j+=1
        ...
        form_res = ir_model_data.get_object_reference(cr, uid, 'pti_wbh_pos_report', 'report_wbh_store_category_view')
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'report.wbh.store.category',
            'res_id': ids[0],
            'view_id': False,
            'views': [(form_id, 'form')],
