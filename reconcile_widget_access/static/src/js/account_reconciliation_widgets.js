...
var bankStatementReconciliation = abstractReconciliation.extend({
    ...
    init: function(parent, context) {
        ...
        if (context.context.statement_line_ids) this.statement_line_ids = context.context.statement_line_ids;        
    ...
    start: function() {
        ...
        return $.when(this._super()).then(function(){
            ...
            if (self.statement_line_ids !== undefined){
                deferred_promises.push(self.model_bank_statement
                    .call("reconciliation_widget_preprocess_with_line", [self.statement_ids || undefined, self.statement_line_ids])
            ...
            return $.when.apply($, deferred_promises).then(function(){
                ...
                return self.model_bank_statement_line
                    ...
                    .then(function (data) {
                        ...
                        if (datum !== undefined)
                            child_promises.push(self.displayReconciliation(datum.st_line.id, 'match', false, true, datum.st_line, datum.reconciliation_proposition, datum.domain_partner_ids));
                        while ((datum = data.shift()) !== undefined)
                            child_promises.push(self.displayReconciliation(datum.st_line.id, 'inactive', false, true, datum.st_line, datum.reconciliation_proposition, datum.domain_partner_ids));
    ...
    displayReconciliation: function(line_id, mode, animate_entrance, initial_data_provided, line, reconciliation_proposition, domain_partner_ids) {
        var self = this;
            ...
            .then(function (data) {
                ...
                while ((datum = data.shift()) !== undefined) {
                    var context = {
                        ...
                        domain_partner_ids: datum.domain_partner_ids,
                    };
...
var bankStatementReconciliationLine = abstractReconciliationLine.extend({
    ...
    createFormWidgets: function() {
        var self = this;
        this._super();
        console.log("self.context.initial_data_provided")
        console.log(self.context.initial_data_provided)
        console.log(self.context)
        console.log(self.context.domain_partner_ids)
        // generate the change partner "form"
        var change_partner_field = {
            relation: "res.partner",
            string: _t("Partner"),
            type: "many2one",
            domain: [['id','in',self.context.domain_partner_ids]],
            help: "",
            readonly: false,
            required: true,
            selectable: true,
