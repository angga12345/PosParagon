...
class AccountBankStatement(models.Model):
    ...
    def button_confirm_bank(self):
        if self.journal_id.default_debit_account_id.id == False:
            raise UserError(_("Default Debit Account id and Default Credit Account id is 'Empty', Please Fill it"))
        else:
            self._balance_check()
    ...
    kategori_shop = fields.Char('Kategori Shop')
...
class AccountBankStatementLine(models.Model):
    ...
    def process_reconciliation(self, counterpart_aml_dicts=None, payment_aml_rec=None, new_aml_dicts=None):
        ...
        if counterpart_aml_dicts or new_aml_dicts:
            ...
            kat_sp = self.statement_id.kategori_shop
            move_vals = self._prepare_reconciliation_move(move_name)
            #akan membuat account.move
            #jika toko tersebut kategorinya adalah stand alone maka buat account.move
            #maka kalau bukan ya tidak membuat apa2
            #if kat_sp != 'stand_alone' or kat_sp != 'shop_in_shop' or kat_sp != 'shop_in_shop_mds':
            if not kat_sp:
