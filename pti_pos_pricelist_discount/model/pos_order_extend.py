...
#     def _set_price_unit(self, lines, order):
#         for line in lines:
#             data = line[2]
#             if data.get('is_special_price'):
#                 count = self._count_special_price(lines, data.get('index_special_price'))
#                 data['price_unit'] = (float(100)/float(110) * order['special_price']) / count

    def _count_special_price(self, lines, index_special_price):
        count = 0
        for line in lines:
            data = line[2]
            if data.get('is_special_price'):
                count = count + data.get('qty')
        return count
