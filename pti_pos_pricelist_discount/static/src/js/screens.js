...
var NumpadWidget = PosBaseWidget.extend({
    ...
    start: function() {
        ...
        this.changedMode();
        //check if user now allow edit price = false then hidden
        if (!this.pos.user.allow_edit_price){
...
var OrderWidget = PosBaseWidget.extend({
    ...
    update_summary: function(){
        var order = this.pos.get_order();
        var has_voucher = order.has_voucher();
        if (!order.get_orderlines().length) {
            return;
        }

//        var total     = order ? order.get_total_with_tax() : 0;
        var subtotal  = order ? order.get_total_with_tax() : 0;
        var taxes     = order ? order.get_total_tax() : 0;
        var voucher   = order ? order.get_voucher_price() : 0;
        var total 	  = order ? order.get_grand_total() : 0;
        var special_discount = order ? order.max_reward_value : 0;

        //console.log("DISCOUNT MEMBERSHIP VALUE")
        //console.log(order.get_total_without_tax() * (order.get_membership_disc() /100))
        //console.log("TOTAL VALUE")
        //console.log(total)
        //console.log("TOTAL WITHOUT TAG")
        //console.log(order.get_total_without_tax())

        if(has_voucher){
            this.el.querySelector('.voucher').textContent = this.format_currency(voucher);
        }

        this.el.querySelector('.taxes').textContent = this.format_currency(taxes);
        this.el.querySelector('.subtotal').textContent = this.format_currency(subtotal);
//        this.el.querySelector('.special_discount').textContent = this.format_currency(special_discount);
    ...
    perform_search: function(category, query, buy_result){
        ...
        if(query){
            ...
            if(buy_result && products.length === 1){
            		console.log('TES 1')
            		console.log('...... ',this.pos.get_order().pos.config.loyalty_id)
            		if(this.pos.get_order().pos.config.loyalty_id != false){
            			console.log('Tambah produk: ',products[0])
            			...
            			if (rewards.length === 0) {
            	    	    ...
            	    	    //console.log('sekarang rewardnya ini:',rewards)
            	    	   ...
            	    	   for(obj in rewards){
            	    		   order.apply_reward(rewards[obj]);
            	    	   }
            	    	   this.clear_search();
            	    	   return;
            	        }
        	...
        	console.log('Produk Scan: ',products)
            this.product_list_widget.set_product_list(products);
            console.log('Hasil List: ',products)
        }
    },
...
var ProductListWidget = PosBaseWidget.extend({
    ...
    render_product: function(product){
    	var prodModel = new Model('product.product');
    	var st_loc = this.pos.config.stock_location_id[0];
        var cached = this.product_cache.get_node(product.id);
        var tmp  = new $.Deferred();
        
        /* fungsi RPC Call Python not used any more */
//        prodModel.call('stock_location_on_hand', [product.id, this.pos.config.stock_location_id[0]]
...
//            	  //console.log('location id',st_loc)
//            	  //console.log('quantity',result)
//	        }, function(err,event){ event.preventDefault(); tmp.reject();
//        });
...
var ProductScreenWidget = ScreenWidget.extend({
    ...
    click_product: function(product) {
       if(product.to_weight && this.pos.config.iface_electronic_scale){
    	   ...
    	   /*ini khusus untuk loyalty yang ada (get discount automatic)*/
    	   if(this.pos.get_order().pos.config.loyalty_id != false){
               ...
               if (rewards.length === 0) {
    	    	   ...
    	    	   for(var reward in rewards){
    	    		   if(rewards[reward].select_manual == false){
		    	    	   	order.apply_reward(rewards[reward]);
		    	        	order.loyalty_reward_id = rewards[reward].id;
		    	        	order.reward = rewards[reward];
    	    		   }
    	            return;
    	    	   }
    	       }
...
var ClientListScreenWidget = ScreenWidget.extend({
    ...
    save_client_details: function(partner) {
        ...
        fields.discount_member_id   = fields.discount_member_id || false;
        //console.log('ffff',fields.discount_member_id)
        fields.ID_card_number	= fields.ID_card_number || '';
        //console.log('gggg',fields.ID_card_number)
        fields.university	= fields.university || '';
        //console.log('hhhh',fields.university)
        fields.community	= fields.community || '';
        //console.log('iiii',fields.community)
...
var ReceiptScreenWidget = ScreenWidget.extend({
    ...
    render_receipt: function() {
        var order = this.pos.get_order();
//        var ordername = order.name.split("-")
//        order.name = ordername[1]
        //console.log("pos config -------------- : ",this.pos.config)
        this.$('.pos-receipt-container').html(QWeb.render('PosTicket',{
                widget:this,
                order: order,
                text : this.pos.config.text,
                ads  : this.pos.config.ads,
...
var PaymentScreenWidget = ScreenWidget.extend({
    ...
    init: function(parent, options) {
		...
		//ini untuk merender dan menampilkan yang sudah ditambahkan customnya
		this.renderElement();
        ...
        this.inputbuffer = "";
        this.inputbatchnobuffer = "";
        this.firstinput  = true;
        this.firstinputbatch = true;
    ...
    reset_input: function(){
        ...
        if (line) {
            ...
            if(line.get_batchno()){
            	this.inputbatchnobuffer = line.get_batchno().toString();
            }else{
            	this.inputbatchnobuffer = ""
            }
        } else {
    ...
    payment_input: function(input) {
        ...
        }else if(this.batch_mode == "true")
        	{
        	    
        		newbufbatch = this.gui.numpad_input(this.inputbatchnobuffer, input, {'firstinputbatch': this.firstinputbatch});
        		this.firstinputbatch = (newbufbatch.length === 0);
        		if (newbufbatch !== this.inputbatchnobuffer) {
    	        	
    	            this.inputbatchnobuffer = newbufbatch;
    	            var order = this.pos.get_order();
    	            if (order.selected_paymentline) {
    	                var batch = this.inputbatchnobuffer;
    	
    	                if (this.inputbatchnobuffer !== "-") {
    	                    batch = this.inputbatchnobuffer;
    	                }
    	
    	                order.selected_paymentline.set_batchno(batch);
    	                this.order_changes();
    	                this.render_paymentlines();
    	                this.$('.paymentline.selected .edit').text(batch);
    	            }
    	        }
        	}      
    },
    ...
    render_numpad: function() {
        var self = this;
        var category_shop = this.pos.config.category_shop;
        var numpad = $(QWeb.render('PaymentScreen-Numpad', {
        	widget:this,
    ...
    render_paymentlines: function() {
        ...
        lines.on('click','.toggle-button',function(){
            ...
            if(self.batch_mode == "true"){
            	...
            	document.getElementById('batchmode').className = 'toggle-button';
            	
            	}
            else if (self.batch_mode == "false"){
            	self.batch_mode = "true" ;
            	document.getElementById('input_batch').className = 'col-tendered edit';
            	document.getElementById('pay').className = '';
            	document.getElementById('batchmode').className = 'toggle-button toggle-button-selected';
            }
        });
    ...
    renderElement: function() {
        ...
        this.render_paymentlines();
		/*ini adalah menampilkan receipt ref nya*/
        var orderrr = this.pos.get_order();
        if (!orderrr){
			return;
		}else{
			this.receipt_ref = this.pos.changed.selectedOrder.name || '';
		}
    ...
    order_changes: function(){
        ...
        if (!order) {
        	...
        	if(order.selected_paymentline != undefined){
	        	if(order.selected_paymentline.cashregister.journal.type == 'bank'){
	        		if(order.selected_paymentline.batchno.length==16){
