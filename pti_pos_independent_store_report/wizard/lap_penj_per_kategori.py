...
class LapPenjPerKategoriReportWizard(osv.osv_memory):
    _name = "lap.penj.per.kategori.idep.store.wizard"
    ...
    def generate_excel_lap_penj_per_kategori(self, cr, uid, ids, context=None):
        if context is None:
            context = {}
        data = {}
    ...
    def _excel_report(self, cr, uid, ids, data, context=None):
        ...
        for x in pos:
            print x.store.name
            shop=x.store.name
        filename = 'Laporan Penjualan Per Kategori '+shop+'.xlsx'
        ...
        from
            ...
            where
            po.date_order >= %s and po.date_order <= %s and pol.price_unit>0 and pc.stand_alone_categ = 'independent') as temp 
        join
        ...
        form_res = ir_model_data.get_object_reference(cr, uid, 'pti_pos_independent_store_report', 'independent_store_report_lap_penj_per_kategori_wizard_view')
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'lap.penj.per.kategori.idep.store.wizard',
            'res_id': ids[0],
            'view_id': False,
            'views': [(form_id, 'form')],
