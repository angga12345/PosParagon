...
class PenjTotalHarianMingguanBulananReportIdepStoreWizard(models.TransientModel):
    _name = "penj.total.harian.idep.store.wizard"

    start_period = fields.Date(string="Start Period")
    end_period = fields.Date(string="End Period")
    store_name = fields.Many2one("pos.config", string="Store", domain=[('category_shop', '=','stand_alone'),('stand_alone_categ', '=', 'independent')])
    ...
    @api.multi
    def generate_excel_penj_total_harian(self):
        for this in self:
            ...
            filename = 'Penj Total Harian '+self.store_name.name+'.xls'
            ...
            worksheet.merge_range('A1:D1', 'Penj. Total Harian '+self.store_name.name, center_title)
...
#                     print stringMerge
            ...
            bz.close()
            form_res = ir_model_data.get_object_reference('pti_pos_independent_store_report', 'independent_store_report_penj_total_harian_wizard_view')
            ...
            return {
                ...
                'view_type': 'form',
                'view_mode': 'form',
                'res_model': 'penj.total.harian.idep.store.wizard',
                'res_id': this.id,
                'view_id': False,
                'views': [(form_id, 'form')],
