...
class IndependentStoreProdukreport(models.TransientModel):
    _name = "independent.store.report.produk.wizard"
    
    store = fields.Many2one('pos.config' , string="Store", domain=[('category_shop', '=','stand_alone'), ('stand_alone_categ', '=','independent')])
...
#         data_obj = [[]]
        self.env.cr.execute("""
            select po.max_reward_value as reward from pos_order po
                join pos_session ps on ps.id = po.session_id
                join pos_config pc on pc.id = ps.config_id
...
#         data_obj = [[]]
    ...
    def _get_data_voucher(self, start_period, end_period):
#         data_obj = [[]]
        self.env.cr.execute("""
    ...
    def generate_excel_produk(self):
        ...
        filename = 'Produk '+self.store.name+'.xlsx'
        ...
        obj_pos_voucher = self._get_data_voucher(start_period, end_period)
        ...
        for max_reward in obj_pos_max_reward:
            worksheet.write(row+7 , 0, number, normal_center)
            worksheet.write(row+7 , 1, 'Cashback', normal_left)                        
        ...
        for special in obj_pos_special_price:
            worksheet.write(row+7 , 0, number, normal_center)
            worksheet.write(row+7 , 1, 'Cashback Special Price', normal_left)                        
        ...
        bz.close()
        form_res = ir_model_data.get_object_reference('pti_pos_independent_store_report', 'independent_store_report_produk_wizard_view')
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'independent.store.report.produk.wizard',
            'res_id': self.id,
            'view_id': False,
            'views': [(form_id, 'form')],
