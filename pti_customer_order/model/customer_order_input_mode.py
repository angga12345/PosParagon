...
class SaleOrderInputMode(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'), ('created', 'Created')], 'State', default='draft')
    ...
    pricelist_id = fields.Many2one('product.pricelist', string='Pricelist', help="Pricelist for current sales order.", default=lambda self: self._get_pricelist_default())
    free_ids = fields.One2many('sale.free.product.list', 'sale_id', 'Free products')
