...
class RekapVoucherReportWizard(models.TransientModel):
    _name = "rekap.voucher.report.wizard"
    ...
    def generate_excel(self):
        ...
        for this in self:
            ...
            filename = 'WBH Rekapitulasi Voucher Per Store '+self.store.name+'.xls'
            ...
            worksheet = workbook.add_worksheet("Rekapitulasi Voucher")
            ...
            worksheet.merge_range('A1:E1', 'REKAPITULASI PENGGUNAAN VOUCHER', center_title)
            worksheet.write('A3','Store Name :',normal_left_bold)
            worksheet.merge_range('A3:B3','Store Name : '+this.store.name, normal_left_bold)
            ...
            self.env.cr.execute("""
                ...
                sum(pline.qty) as pcs,sum(round(abs(ptemplate.list_price)*pline.qty)) as jumlah,ptemplate.voucher
                    ...
                    where porder.date_order >= %s and porder.date_order <= %s and pconfig.id = %s and ptemplate.voucher=true
                    group by
                ...
                """, (start_period+' 00:00:00', end_period+' 23:59:59',sn))
            ...
            bz.close()
            form_res = ir_model_data.get_object_reference('rekap_voucher', 'rekap_voucher_report_wizard_view')
            ...
            return {
                ...
                'view_type': 'form',
                'view_mode': 'form',
                'res_model': 'rekap.voucher.report.wizard',
                'res_id': this.id,
                'view_id': False,
                'views': [(form_id, 'form')],
